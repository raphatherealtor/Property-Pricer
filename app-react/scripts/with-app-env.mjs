...
function resolveSpawnCommand(command, args, root) {
  if (process.platform === "win32" && command === "vite") {
    const viteCli = join(root, "node_modules", "vite", "bin", "vite.js");
    if (existsSync(viteCli)) return { command: process.execPath, args: [viteCli, ...args] };
  }
  return { command, args };
}
...
function main(argv) {
  ...
  const resolved = resolveSpawnCommand(command, args, root);
